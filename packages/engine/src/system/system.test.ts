...
describe("system trait filtering", () => {
  it("system only runs on entities with all required traits", () => {
    ...
    world.dispatch({ type: "entity.create", params: { entityType: "mover", id: "m1" } })
    world.dispatch({
      type: "trait.update",
    ...
    world.dispatch({ type: "entity.create", params: { entityType: "mover", id: "m2" } })
    world.dispatch({
      type: "trait.update",
    ...
    world.dispatch({ type: "entity.create", params: { entityType: "static", id: "s1" } })
...
describe("system emitting intents", () => {
  it("system can mutate world state by returning intents", () => {
              ...
              return [
                {
                  ...
                  params: {
                    entityId: e.id,
                    traitName: "position",
                    data: { x: pos.x + vel.vx * dt, y: pos.y + vel.vy * dt },
                  },
                },
              ]
            })
    ...
    const world = World.create(schema)
    world.dispatch({ type: "entity.create", params: { entityType: "ball", id: "ball1" } })
    world.dispatch({
      type: "trait.update",
    ...
    const pos = world.query.byId("ball1")?.traits["position"] as { x: number; y: number }
    expect(pos.x).toBeCloseTo(10)
    expect(pos.y).toBeCloseTo(5)

    world.tick(1.0) // another second
    const pos2 = world.query.byId("ball1")?.traits["position"] as { x: number; y: number }
  ...
  it("multiple ticks accumulate correctly", () => {
        ...
        defineSystem({
          ...
          update(entities): IntentInput[] {
            return entities.map((e) => {
              const current = (e.traits["counter"] as { value: number }).value
              return {
                type: "trait.update",
                params: {
                  entityId: e.id,
    ...
    const world = World.create(schema)
    world.dispatch({ type: "entity.create", params: { entityType: "ticker", id: "t1" } })
    world.dispatch({
      type: "trait.update",
                ...
                violations: [
                  {
                    ...
                    message: "System updates blocked by test constraint",
                    entityIds: [],
                    effect: "prevent",
                  },
                ],
        ...
        defineSystem({
          ...
          update(entities): IntentInput[] {
            return entities.map((e) => {
              ...
              return {
                type: "trait.update",
                params: { entityId: e.id, traitName: "counter", data: { value: v + 1 } },
              }
            })
          },
        }),
...
describe("SystemWorldView in update", () => {
  it("provides query access to world state", () => {
    ...
    const world = World.create(schema)
    world.dispatch({ type: "entity.create", params: { entityType: "target", id: "t1" } })
    world.dispatch({ type: "entity.create", params: { entityType: "target", id: "t2" } })
    world.dispatch({ type: "entity.create", params: { entityType: "other", id: "o1" } })
