...
describe("World.create", () => {
  ...
  it("schema traits are registered and validated", () => {
    ...
    expect(() =>
      world.dispatch({
        type: "trait.update",
        params: { entityId: "b1", traitName: "color", data: { r: 999, g: 0, b: 0 } },
      }),
    ).toThrow()
  })
  ...
  it("multiple worlds are independent (no shared singletons)", () => {
    ...
    worldB.dispatch({ type: "entity.create", params: { entityType: "box", id: "b1" } })
    expect(worldA.query.count).toBe(1)
...
describe("World.query", () => {
  ...
  beforeEach(() => {
    world = World.create(testSchema)
    world.dispatch({ type: "entity.create", params: { entityType: "wall", id: "w1" } })
    world.dispatch({ type: "entity.create", params: { entityType: "wall", id: "w2" } })
...
describe("World.serialize / deserialize", () => {
  ...
  it("snapshot captures trait data", () => {
    ...
    world.dispatch({
      type: "trait.update",
      params: { entityId: "b1", traitName: "color", data: { r: 200, g: 100, b: 50 } },
    })
  ...
  it("snapshot captures parent/child relationships", () => {
    world.dispatch({ type: "entity.create", params: { entityType: "group", id: "g1" } })
    world.dispatch({
      type: "entity.create",
  ...
  it("deserialize restores trait data", () => {
    ...
    world.dispatch({
      type: "trait.update",
      params: { entityId: "b1", traitName: "color", data: { r: 42, g: 84, b: 168 } },
    })

    const snap = world.serialize()
    const restored = World.deserialize(snap, testSchema)

    expect(restored.query.byId("b1")?.traits["color"]).toEqual({ r: 42, g: 84, b: 168 })
  })
  ...
  it("restored world is fully functional — can dispatch further intents", () => {
    ...
    const restored = World.deserialize(snap, testSchema)
    restored.dispatch({ type: "entity.create", params: { entityType: "wall", id: "w1" } })
    expect(restored.query.count).toBe(2)
  })
