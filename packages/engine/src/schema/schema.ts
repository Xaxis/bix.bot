...
/**
 * A grouping of entity types shown together in the editor palette.
 * Category names appear as section headers; types are the entity type
 * strings declared in entityTypes.
 */
export interface PaletteCategory {
    /** Display name for this grouping (e.g., "Structure", "Openings"). */
    readonly name: string
    /** Entity type identifiers to show in this category. */
    readonly types: readonly string[]
    /** Optional lucide icon name for the category header. */
...
export type ViewportRenderer = "svg" | "three" | "canvas"
export type ViewportType = "2d" | "3d"

/**
 * Configures a viewport shown in the editor.
 * Multiple viewports can coexist (e.g., 3D scene + 2D floor plan).
 */
export interface ViewportConfig {
    /** "2d" or "3d" — determines which Viewport component is rendered. */
    readonly type: ViewportType
    /** Human-readable tab or panel label. */
    readonly label: string
    /** Rendering backend for this viewport. */
...
export function defineEntityType(config: EntityTypeDefinition): EntityTypeDefinition {
    ...
    if (config.label.trim().length === 0) {
        throw new Error(
            `EntityTypeDefinition for "${config.type}": label must not be empty`,
        )
    }
    return config
}
...
export interface DomainSchema {
    ...
    /**
     * Editor palette configuration.
     * Groups entity types into named categories shown as sections in the
     * Palette component. If omitted, the editor falls back to a flat list
     * of all entityTypes.
     */
    readonly palette?: PaletteConfig

    /**
     * Viewport configurations for the editor.
     * Determines which viewports are rendered and in what order.
     * If omitted, the editor shows the default 3D + 2D pair.
     */
