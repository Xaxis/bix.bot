...
export function validateSchema(schema: DomainSchema): SchemaValidationResult {
    ...
    // ── palette ───────────────────────────────────────────────────
    // Only validate palette category types if entityTypes is declared
    if (entityTypeNames.size > 0 && schema.palette) {
        for (let i = 0; i < schema.palette.categories.length; i++) {
            const cat = schema.palette.categories[i]!
            for (let j = 0; j < cat.types.length; j++) {
                const t = cat.types[j]!
                if (!entityTypeNames.has(t)) {
                    errors.push({
                        code: "UNKNOWN_PALETTE_ENTITY_TYPE",
                        field: `palette.categories[${i}].types[${j}]`,
                        message: `Palette category "${cat.name}" references unknown entity type "${t}". Declare it in schema.entityTypes first.`,
                    })
                }
            }
        }
