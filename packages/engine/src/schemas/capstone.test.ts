...
describe("validateSchema — test schema is valid", () => {
    ...
    it("catches constraint watch.entityTypes referencing undeclared type", () => {
        ...
        const result = validateSchema({
            ...
            traits: [],
            entityTypes: [
                { type: "real-type", label: "Real", description: "", traits: [] },
            ],
            constraints: [c],
        })
        expect(result.valid).toBe(false)
...
describe("entity placement", () => {
    ...
    it("creates a Label entity", () => {
        world.dispatch({
            type: "entity.create",
            params: { entityType: "label", id: "lbl1" },
        })
    ...
    it("attaches traits to Box", () => {
        ...
        expect(
            (world.query.byId("box1")?.traits["color"] as { value: string })?.value,
        ).toBe("blue")
...
describe("constraint enforcement", () => {
    ...
    it("box-min-size: blocks size update below minimum", () => {
        ...
        // State unchanged — size remains at auto-default (update was prevented)
        const sizeAfter = world.query.byId("box1")?.traits["size"] as Size | undefined
        expect(sizeAfter?.width).toBe(1) // auto-default preserved, not 0.5
    })
...
describe("undo / redo", () => {
    ...
    it("undo reverses trait update", () => {
        ...
        world.undo()
        // With auto-defaults, undo restores to the auto-applied default (not undefined)
        expect(
            (world.query.byId("box1")?.traits["color"] as { value: string }).value,
        ).toBe("white")
    })
...
describe("queries", () => {
    ...
    it("query.byTrait() finds by trait name", () => {
        ...
        // With auto-defaults both boxes get the size trait on creation
        expect(world.query.byTrait("size")).toHaveLength(2)
    })

    it("query.withTraits() requires all traits", () => {
        // Both boxes get color+size from auto-defaults; test the count (explicit update on box1 still differs)
        expect(world.query.withTraits(["color", "size"])).toHaveLength(2)
        // box1 has the explicitly-updated size (2,2,2); use size value to distinguish
    })
...
describe("system tick", () => {
    ...
    it("tick does not affect undo stack directly", () => {
        ...
        world.dispatch({
            type: "entity.create",
            params: { entityType: "label", id: "lbl2" },
        })
        world.dispatch({
            type: "trait.update",
            params: {
...
describe("full scene lifecycle", () => {
    it("build → constrain → serialize → restore → continue", () => {
        ...
        world.dispatch({
            type: "entity.create",
            params: { entityType: "box", id: "shelf" },
        })
        world.dispatch({
            type: "trait.update",
        ...
        world.dispatch({
            type: "entity.create",
            params: { entityType: "label", id: "tag" },
        })
