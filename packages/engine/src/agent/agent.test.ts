...
describe("generateToolDefinitions", () => {
    ...
    it("create_box parameters include size trait schema", () => {
        ...
        expect(sizeParam?.properties?.["height"]).toEqual({
            type: "number",
            minimum: 0.1,
        })
...
describe("executeAgentTool — query tools", () => {
    ...
    it("list_entities returns entity summaries with traitNames", () => {
        ...
        const data = result.data as {
            entities: Array<{ id: string; traitNames: string[] }>
        }
    ...
    it("get_connections returns parent and children", () => {
        const result = executeAgentTool(world, testSchema, "get_connections", {
            id: "box1",
        })
        ...
        const schema = defineSchema({
            ...
            traits: [
                defineTrait({
                    ...
                    editable: {
                        fields: {
                            celsius: { label: "Temperature (°C)", widget: "slider" },
                        },
                    },
                }),
            ],
