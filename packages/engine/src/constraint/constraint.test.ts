...
describe("constraint effect: prevent", () => {
          ...
          evaluate(ctx) {
            ...
            if (
              ...
              const traits = intent.params["traits"] as
                ...
                | undefined
              const length =
                (traits?.["length"] as { meters: number } | undefined)?.meters ?? 1
              if (length < 0.5) {
                return {
                  valid: false,
                  violations: [
  ...
  it("can prevent entity.delete by inspecting entity before deletion", () => {
      ...
      constraints: [
        defineConstraint({
          ...
          evaluate(ctx) {
            ...
            const currentLoad =
              (entity?.traits["load"] as { kg: number } | undefined)?.kg ?? 0
    ...
    const world = World.create(schema)
    world.dispatch({ type: "entity.create", params: { entityType: "shelf", id: "s1" } })
    world.dispatch({
      type: "trait.update",
...
describe("constraint effect: adjust", () => {
  it("dispatches fix intents after execution", () => {
              ...
              if (id) {
                return {
                  ...
                  suggestions: [
                    {
                      type: "trait.update",
                      params: { entityId: id, traitName: "counter", data: { value: 42 } },
    ...
    const world = World.create(schema)
    world.dispatch({ type: "entity.create", params: { entityType: "counted", id: "c1" } })
            ...
            return {
              ...
              suggestions: ctx.entities.byTrait("counter").map((e) => ({
                ...
                params: {
                  ...
                  data: {
                    value:
                      Math.round((e.traits["counter"] as { value: number }).value / 10) *
                      10,
                  },
                },
                source: "system" as const,
...
describe("global vs local scope", () => {
  ...
  it("local constraint only fires for watched entity types", () => {
    ...
    const schema = defineSchema({
      ...
      constraints: [
        defineConstraint({
          ...
          watch: { entityTypes: ["wall"] },
          evaluate(ctx) {
            evaluations.push(ctx.trigger.intent.params["entityType"] as string)
            return { valid: true, violations: [] }
          },
        }),
      ],
    ...
    const world = World.create(schema)
    world.dispatch({ type: "entity.create", params: { entityType: "door", id: "d1" } })
  ...
  it("constraint checks relationships between multiple entities", () => {
    ...
    const schema = defineSchema({
      ...
      constraints: [
        defineConstraint({
          name: "shelf-capacity",
          description: "Total load on shelf children must not exceed shelf capacity",
          priority: 20,
          scope: "local",
          effect: "prevent",
          ...
          evaluate(ctx) {
            const intent = ctx.trigger.intent
            if (intent.type !== "trait.update" || intent.params["traitName"] !== "load") {
              return { valid: true, violations: [] }
            }
            ...
            const maxKg =
              (shelf.traits["capacity"] as { maxKg: number } | undefined)?.maxKg ??
              Infinity
            ...
            for (const childId of shelf.children) {
              ...
              totalLoad +=
                (sibling?.traits["load"] as { kg: number } | undefined)?.kg ?? 0
            }
...
describe("context-sensitive constraint", () => {
  it("constraint queries world state to make its decision", () => {
                ...
                violations: [
                  {
                    ...
                    message: "Cannot create attachment — no host entity exists",
                    entityIds: [],
                    effect: "prevent",
                  },
                ],
    ...
    // Create a host
    world.dispatch({ type: "entity.create", params: { entityType: "host", id: "h1" } })
